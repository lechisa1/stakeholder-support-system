    ...
    <g onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      ...
      {hovered && assignedUsers.length > 0 && (
        ...
        >
          ...
          >
            <p className="text-sm font-semibold text-center text-[#094C81]">
              Assigned Users
            </p>
  ...
  const buildTree = (nodes: TreeNode[]): TreeNode[] => {
    ...
    nodes.forEach((node) => {
      if (node.parent_id) {
        ...
        if (parent) parent.children!.push(nodeMap.get(node.hierarchy_node_id)!);
      } else {
        tree.push(nodeMap.get(node.hierarchy_node_id)!);
      }
    });
    return tree;
  };
      ...
      >
        ...
        {selectedD3TreeData ? (
          <>
            ...
            <div className="absolute top-4 right-4 bg-white rounded-lg shadow-lg p-2 border border-gray-200 text-xs text-gray-600 space-y-1">
