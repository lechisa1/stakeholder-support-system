...
export default function UserAddressCard() {
  ...
  // Controlled state for address fields
  const [country, setCountry] = useState(user.address?.country || "United States");
  const [city, setCity] = useState(user.address?.city || "Phoenix, Arizona, United States");
  const [postalCode, setPostalCode] = useState(user.address?.postalCode || "ERT 2489");
  const [taxId, setTaxId] = useState(user.address?.taxId || "AS4568384");
        ...
        <div className="relative w-full p-4 overflow-y-auto bg-white no-scrollbar rounded-3xl dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Update your address to keep your profile up-to-date.
            </p>
          </div>
